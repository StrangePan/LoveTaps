...
vec4 effect(vec4 color, sampler2D t, vec2 texture_coords, vec2 screen_coords) {
    ...
    if (screen_coords.y < sin((screen_coords.x + time * 20) / 20) * 15 + 100 * s) {
        tint = topColor;
    } else if (screen_coords.y < sin((screen_coords.x + 20 - time * 20) / 20) * 15 + 200 * s) {
        tint = mix(topColor, bottomColor, 0.2);
    } else if (screen_coords.y < sin((screen_coords.x + 60 + time * 20) / 20) * 15 + 300 * s) {
        tint = mix(topColor, bottomColor, 0.4);
    } else if (screen_coords.y < sin((screen_coords.x + 90 - time * 20) / 20) * 15 + 400 * s) {
        tint = mix(topColor, bottomColor, 0.6);
    } else if (screen_coords.y < sin((screen_coords.x + 50 + time * 20) / 20) * 15 + 500 * s) {
